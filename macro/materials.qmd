...
def pct(v):
    return f"{v*100:.1f}%" if v is not None and str(v) != 'nan' else "N/A"
    ...
    for i, (ticker, name) in enumerate(stocks.items()):
        try:
            ...
            ax.plot(normalized.index, normalized.values,
                    label=f"{name} ({ticker})", color=colors[i], linewidth=1.8)
        except Exception:
            continue

    ax.axhline(100, color='gray', linestyle='--', linewidth=0.8, alpha=0.6)
    ax.set_title(group, fontsize=13, fontweight='bold', pad=10)
    ax.set_ylabel('相対パフォーマンス（基準=100）', fontsize=9)
...
fig.suptitle('素材セクター：株価パフォーマンス比較（過去1年、基準=100）',
             fontsize=14, fontweight='bold', y=1.01)
plt.tight_layout()
plt.savefig('materials_performance.png', dpi=150, bbox_inches='tight')
plt.show()
```
...
if margins:
    ...
    colors_bar = ['#2196F3' if v >= 20 else '#FF9800' if v >= 10 else '#F44336' for v in values]

    fig, ax = plt.subplots(figsize=(12, 5))
    bars = ax.barh(labels, values, color=colors_bar, alpha=0.85)
    ax.axvline(0, color='black', linewidth=0.8)
    ax.set_xlabel('営業利益率（%）', fontsize=10)
    ax.set_title('営業利益率の比較（青≥20%、オレンジ≥10%、赤<10%）',
